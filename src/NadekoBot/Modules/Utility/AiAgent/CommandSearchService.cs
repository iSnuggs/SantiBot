...
public sealed class CommandSearchService(
    ...
    AiAgentConfigService configService) : INService, IReadyExecutor
{
    ...
    public CommandSearchResult[] Search(string query, int topK = 5)
    {
        ...
        Log.Information("CommandSearch: query='{Query}' -> {Results}",
            query,
            string.Join(", ", results.Select(r => $"{r.Command.Aliases[0]} ({r.Score:F3})")));
    ...
    public bool IsCommandIntent(string normalizedText)
    {
        ...
        Log.Information("IntentClassify(command): '{Text}' -> {Result}",
            normalizedText.TrimTo(80), result ? "TRIGGER" : "IGNORE");

        return result;
    }

    /// <summary>
    /// Classify whether a message in a follow-up window is a follow-up to the bot conversation
    /// or just general chatter. Returns true if it's likely a follow-up.
    /// </summary>
    public bool IsFollowUpIntent(string text)
    {
        if (!_ready || !_headReady)
            return false;
        ...
        Log.Information("IntentClassify(followup): '{Text}' -> {Result}",
            text.TrimTo(80), result ? "TRIGGER" : "IGNORE");
    ...
    private async Task EnsureModelDownloadedAsync()
    {
        ...
        {
            var url = configService.Data.EmbeddingModelUrl;
