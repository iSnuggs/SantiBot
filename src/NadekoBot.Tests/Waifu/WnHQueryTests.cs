...
public class WnHQueryTests
{
    ...
    public void Setup()
    {
        ...
        _svc = new WnHService(_db, _cache, _cs, client, null!, time);
    ...
    public async Task GetWaifuInfo_OptedIn_ReturnsFullDto()
    {
        await using (var ctx = _db.GetDbContext())
            await WnHTestHelper.CreateWaifu(ctx, 1001, mood: 800, food: 600, fee: 3, price: 5000,
    ...
    public async Task IsWaifu_IsFan_IsManager_ReturnCorrectBooleans()
    {
        await using (var ctx = _db.GetDbContext())
        {
            await WnHTestHelper.CreateWaifu(ctx, 1001, managerId: 5001);
            await WnHTestHelper.CreateFan(ctx, 2001, 1001);
        }
    ...
    {
        await using (var ctx = _db.GetDbContext())
            await WnHTestHelper.CreateWaifu(ctx, 1001, price: 10_000, managerId: 5001);

        var result = await _svc.GetManagerExitInfoAsync(5001);
        Assert.That(result, Is.Not.Null);
        ...
        Assert.That(await _svc.GetManagerExitInfoAsync(9999), Is.Null);
    }
    ...
    public async Task GetFans_WithCycleHistory_ReturnsEarningsAndPending()
    {
        ...
        await using (var ctx = _db.GetDbContext())
        {
            ...
            await WnHTestHelper.CreateCycleSnapshot(ctx, currentCycle, 1001, 2001, 5000);

            // Last cycle data
            await WnHTestHelper.CreateCycleSnapshot(ctx, lastCycle, 1001, 2001, 5000);
            await WnHTestHelper.CreateCycleSnapshot(ctx, lastCycle, 1001, 3001, 3000);
            await WnHTestHelper.CreateCycleRecord(ctx, 1001, lastCycle, 5001,
    ...
    [Test]
    public async Task GetLeaderboard_RankedByRealizedReturnRate()
    {
        var currentCycle = _svc.GetCurrentCycle();
        ...
        {
            await WnHTestHelper.CreateWaifu(ctx, 1001);
            await WnHTestHelper.CreateCycleRecord(ctx, 1001, currentCycle - 1, 5001,
                totalBacked: 10000, totalReturns: 500, waifuEarnings: 20, managerEarnings: 5, fanPool: 475);

            await WnHTestHelper.CreateWaifu(ctx, 2001);
            await WnHTestHelper.CreateCycleRecord(ctx, 2001, currentCycle - 1, 6001,
                totalBacked: 20000, totalReturns: 200, waifuEarnings: 8, managerEarnings: 2, fanPool: 190);
        }

        var result = await _svc.GetLeaderboardAsync(10);
        Assert.That(result, Has.Count.EqualTo(2));
        Assert.That(result[0].UserId, Is.EqualTo(1001));
        Assert.That(result[0].RealizedReturnRate, Is.GreaterThan(result[1].RealizedReturnRate));
    }
    ...
    public async Task GetProjectedPayout_WithSnapshotData_ReturnsCorrectProjection()
    {
        ...
        await using (var ctx = _db.GetDbContext())
        {
            await WnHTestHelper.CreateWaifu(ctx, 1001, mood: 1000, food: 1000, fee: 5, managerId: 5001,
                returnsCap: 1_000_000_000);
            await WnHTestHelper.CreateCycleSnapshot(ctx, currentCycle, 1001, 2001, 100_000_000);
        }
