...
public class WnHGiftTests
{
    ...
    private static WaifuGiftItem GetTodaysFoodItem()
        => WaifuGiftItems.GetTodaysItems().First(x => x.Type == GiftItemType.Food);

    private static WaifuGiftItem GetTodaysMoodItem()
        => WaifuGiftItems.GetTodaysItems().First(x => x.Type == GiftItemType.Mood);
    ...
    public async Task Gift_FoodAndMood_CorrectStatIncrease()
    {
        ...
        {
            await WnHTestHelper.CreateWaifu(ctx, 1001, food: 100, mood: 200);
        }
    ...
    public async Task Gift_StatsCappedAt1000()
    {
        var food = GetTodaysFoodItem();
        await using (var ctx = _db.GetDbContext())
            await WnHTestHelper.CreateWaifu(ctx, 1001, food: 990);
    ...
    public async Task Gift_PersistsGiftCount_IncrementOnRepeat()
    {
        ...
        await using (var ctx = _db.GetDbContext())
            await WnHTestHelper.CreateWaifu(ctx, 1001, food: 100, mood: 100);
