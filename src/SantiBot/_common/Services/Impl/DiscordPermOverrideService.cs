...
public class DiscordPermOverrideService : INService, IExecPreCommand, IDiscordPermOverrideService
{
    ...
    public bool TryGetOverrides(ulong guildId, string commandName, out SantiBot.Db.GuildPerm? perm)
    {
    ...
    public async Task AddOverride(ulong guildId, string commandName, GuildPerm perm)
    {
        ...
        if (over is null)
        {
            uow.Set<DiscordPermOverride>()
                ...
                {
                    Command = commandName,
                    Perm = (SantiBot.Db.GuildPerm)perm,
                    GuildId = guildId
                });
        }
        else
            over.Perm = (SantiBot.Db.GuildPerm)perm;
