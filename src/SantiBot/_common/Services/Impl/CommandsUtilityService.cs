...
public sealed class CommandsUtilityService : ICommandsUtilityService, INService
{
    ...
    public EmbedBuilder GetCommandHelp(CommandInfo com, IGuild guild)
    {
        ...
        var opt = GetSantiOptionType(com.Attributes);
        if (opt is not null)
        {
    ...
    public static Type? GetSantiOptionType(IEnumerable<Attribute> attributes)
        => attributes
           .Select(a => a.GetType())
           .Where(a => a.IsGenericType
                       && a.GetGenericTypeDefinition() == typeof(SantiOptionsAttribute<>))
