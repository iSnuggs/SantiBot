...
public partial class ResponseBuilder
{
    public class PaginationSender<T>
    {
        ...
        private SantiButtonInteractionHandler? left;
        private SantiButtonInteractionHandler? right;
        private SantiInteractionBase? extra;
        ...
        public async Task SendAsync(bool ephemeral = false)
        {
            ...
            SantiInteractionBase? maybeInter = null;

            var model = await _builder.BuildAsync(ephemeral);

            async Task<(SantiButtonInteractionHandler left, SantiInteractionBase? extra, SantiButtonInteractionHandler right)>
                GetInteractions()
            {
                ...
                var leftBtnInter = new SantiButtonInteractionHandler(_client,
                    model.User?.Id ?? 0,
                    leftButton,
                    (smc) =>
                    {
                ...
                var rightBtnInter = new SantiButtonInteractionHandler(_client,
                    model.User?.Id ?? 0,
                    rightButton,
                    (smc) =>
                    {
