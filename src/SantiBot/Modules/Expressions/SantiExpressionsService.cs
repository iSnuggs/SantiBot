...
using SantiBot.Modules.Permissions.Services;
using YamlDotNet.Serialization;
using YamlDotNet.Serialization.NamingConventions;

namespace SantiBot.Modules.SantiExpressions;

public sealed class SantiExpressionsService : IExecOnMessage, IReadyExecutor, INService
{
    ...
    private readonly TypedKey<SantiExpression> _gexprAddedKey = new("gexpr.added");
    private readonly TypedKey<int> _gexprDeletedkey = new("gexpr.deleted");
    private readonly TypedKey<SantiExpression> _gexprEditedKey = new("gexpr.edited");
    ...
    private SantiExpression[] globalExpressions = Array.Empty<SantiExpression>();
    private ConcurrentDictionary<ulong, SantiExpression[]> newguildExpressions = new();
    ...
    public SantiExpressionsService(
        DbService db,
        IBotStrings strings,
        IBot bot,
    ...
    private async Task ReloadInternal()
    {
        ...
        var guildItems = await uow.GetTable<SantiExpression>()
                                  .AsNoTracking()
        ...
        {
            var globalItems = uow.Set<SantiExpression>()
                                 .AsNoTracking()
    ...
    private SantiExpression TryGetExpression(IUserMessage umsg)
    {
        if (!_isReady)
            return null;
    ...
    private SantiExpression MatchExpressions(in ReadOnlySpan<char> content, SantiExpression[] exprs)
    {
        var result = new List<SantiExpression>(1);
    ...
    {
        SantiExpression expr;
        await using var uow = _db.GetDbContext();
        expr = uow.Set<SantiExpression>().GetById(id);
        if (expr is null)
            return;
    ...
    private Task UpdateInternalAsync(ulong? maybeGuildId, SantiExpression expr)
    {
    ...
    private void UpdateInternal(ulong? maybeGuildId, SantiExpression expr)
    {
        if (maybeGuildId is { } guildId)
        {
    ...
    private Task AddInternalAsync(ulong? maybeGuildId, SantiExpression expr)
    {
    ...
    private SantiExpression[] DeleteInternal(
        IReadOnlyList<SantiExpression> exprs,
        int id,
        out SantiExpression deleted)
    {
        deleted = null;
        if (exprs is null || exprs.Count == 0)
            return exprs as SantiExpression[] ?? exprs?.ToArray();

        var newExprs = new SantiExpression[exprs.Count - 1];
    ...
    public SantiExpression GetExpression(ulong? guildId, int id)
    {
        using var uow = _db.GetDbContext();
        var expr = uow.Set<SantiExpression>().GetById(id);
        if (expr is null || expr.GuildId != guildId)
            return null;
    ...
    {
        using var uow = _db.GetDbContext();
        var count = uow.Set<SantiExpression>().ClearFromGuild(guildId);
        uow.SaveChanges();
    ...
    private ValueTask OnGexprAdded(SantiExpression c)
    {
        lock (_gexprWriteLock)
        {
            var newGlobalReactions = new SantiExpression[globalExpressions.Length + 1];
    ...
    private ValueTask OnGexprEdited(SantiExpression c)
    {
        lock (_gexprWriteLock)
        {
    ...
    public async Task<SantiExpression> AddAsync(
        ulong? guildId,
        string key,
        string message,
        bool ca = false,
        ...
        bool dm = false)
    {
        ...
        await using (var uow = _db.GetDbContext())
        {
            uow.Set<SantiExpression>().Add(expr);
            await uow.SaveChangesAsync();
        }
    ...
    public async Task<SantiExpression> EditAsync(
        ulong? guildId,
        int id,
        string message,
        bool? ca = null,
    ...
    public async Task<SantiExpression> DeleteAsync(ulong? guildId, int id)
    {
        await using var uow = _db.GetDbContext();
        var toDelete = uow.Set<SantiExpression>().GetById(id);
        ...
        {
            uow.Set<SantiExpression>().Remove(toDelete);
            await uow.SaveChangesAsync();
    ...
    [MethodImpl(MethodImplOptions.AggressiveInlining)]
    public SantiExpression[] GetExpressionsFor(ulong? maybeGuildId)
    {
        if (maybeGuildId is { } guildId)
            return newguildExpressions.TryGetValue(guildId, out var exprs) ? exprs : Array.Empty<SantiExpression>();
    ...
    public async Task<(IReadOnlyCollection<SantiExpression> Exprs, int TotalCount)> FindExpressionsAsync(
        ulong guildId,
        string query,
        int page)
    {
