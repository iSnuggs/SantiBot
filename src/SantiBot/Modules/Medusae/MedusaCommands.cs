...
public partial class Owner
{
    ...
    [OwnerOnly]
    [NoPublicBot]
    public partial class Medusa : SantiModule<IMedusaLoaderService>
    {
        ...
        [Cmd]
        [OwnerOnly]
        public async Task MedusaReload(string name)
        {
            if (string.IsNullOrWhiteSpace(name))
            {
                await Response().Error(strs.medusa_not_loaded).SendAsync();
                return;
            }

            var unloadResult = await _service.UnloadMedusaAsync(name);
            if (unloadResult is not (MedusaUnloadResult.Success or MedusaUnloadResult.PossiblyUnable))
            {
                await Response().Error(strs.medusa_not_loaded).SendAsync();
                return;
            }

            var loadResult = await _service.LoadMedusaAsync(name);
            if (loadResult == MedusaLoadResult.Success)
                await Response().Confirm(strs.medusa_reloaded(Format.Code(name))).SendAsync();
            else
                await Response().Error(strs.medusa_reload_failed(Format.Code(name))).SendAsync();
        }
    }
}
